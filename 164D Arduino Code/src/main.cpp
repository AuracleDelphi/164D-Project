...
void getAndDisplayBPM() {
  ...
  if(irValue > threshold){                                          //If finger is placed and detected
    /** Your code goes below here: Start **/    
    ...
    display.println(String(beatAvg));;            //Print beatAvg on display
  ...
  if (irValue < threshold){       //If no finger is detected it inform the user and put the average BPM to 0 or it will be stored for the next measure
     beatAvg=0;
     display.clearDisplay();
     display.setTextSize(1);                    
     display.setTextColor(WHITE);  
